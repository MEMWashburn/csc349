...
public class ChangeMaker {
    ...
    public static int[] change_DP(int n, int[] d) {
        ...
        for (int i = 1; i <= n; i++)  {
            C[i] = Integer.MAX_VALUE;   // To check if visited or not
...
//                            minIndex = i;
//                        }
//                    } else if (minCount > C[j - d[i]]) {
//                        System.out.println("minCount > C[j - d[i]: " + minCount);
//                        minCount = C[j - d[i]];
//                        minIndex = i;
//                    }
//
...
//        }
        ...
        for (int j = 1; j <= n; j++) {
            for (int i = 0; i < k; i++) {
                if (j >= d[i]) {                // Avoiding negative change
                    int potential = C[j - d[i]];
                    if ( (potential != Integer.MAX_VALUE) && (potential + 1 < C[j]) ) {
                        C[j] = potential + 1;   // Plus one more coin
                        A[j] = i;               // Saving which index was chosen as ideal
                    }
                }
            }
        }
